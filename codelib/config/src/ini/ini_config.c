...
static int ini_set_string(struct config *c, ...)
{
    ...
    va_start(ap, c);
    va_arg_type(ap, mix);
    while (mix.type != TYPE_EMPTY && cnt < 2) {//last argument must be NULL
        cnt ++;
    ...
    // free(type_list);
    return iniparser_set(ini, type_list[cnt-2].cval, type_list[cnt-1].cval);
}

static const char *ini_get_string(struct config *c, ...)
{
    dictionary *ini = (dictionary *)c->priv;
