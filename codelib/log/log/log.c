...
#define SAVE_LOGFILE_CNT    3  // 保存以前3个日志文件
static char *save_logfile_array[SAVE_LOGFILE_CNT];
static int save_logfile_cnt = 0;
...
static FILE *save_logfile(FILE *fp)
{
    ...
    if (save_logfile_cnt >= SAVE_LOGFILE_CNT) {
        int i = 0;
        if (-1 == remove(save_logfile_array[0])) {
            fprintf(stderr, "remove `%s' failed: %s\n", save_logfile_array[0],
                    strerror(errno));
        }
        free(save_logfile_array[0]);
        for (; i < save_logfile_cnt-1; i++) {
            save_logfile_array[i] = save_logfile_array[i+1];
        }
        save_logfile_array[save_logfile_cnt-1] = strdup(new_filename);
    }
    else {
        save_logfile_array[save_logfile_cnt++] = strdup(new_filename);
    }
        ...
        const char *errstr, const char *fmtbuf)
{
    ...
    if ( g_fp != stderr ) {
            ...
            len += snprintf(buff + len, sizeof(buff)-len-1, " [%s]", cur_time);
        ...
        case GH_LOG_WARN:
            ...
            0 len += snprintf(buff + len, sizeof(buff)-len-1, " [%s]", cur_time);
